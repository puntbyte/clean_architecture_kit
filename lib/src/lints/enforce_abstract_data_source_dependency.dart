...
class EnforceAbstractDataSourceDependency extends DartLintRule {
  ...
  EnforceAbstractDataSourceDependency({required this.config, required this.layerResolver})
    : super(code: _code);
  ...
  void run(CustomLintResolver resolver, DiagnosticReporter reporter, CustomLintContext context) {
    ...
    context.registry.addConstructorDeclaration((node) {
      ...
      for (final parameter in node.parameters.parameters) {
        ...
        if (validateName(name: typeName, template: template)) {
          ...
          final pattern = template.replaceAll('{{name}}', r'([A-Z][a-zA-Z0-9]+)');
