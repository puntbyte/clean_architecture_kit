  ...
  group('NamingUtils', () {
    ...
    group('validateName', () {
      test('should return true for a valid suffix template match', () {
        expect(NamingUtils.validateName(name: 'GetUserUsecase', template: '{{name}}Usecase'), isTrue);
      });
      ...
      test('should return true for a valid regex template match', () {
        expect(NamingUtils.validateName(name: 'AuthRepositoryImpl', template: '{{name}}(Impl|RepositoryImpl)'), isTrue);
      });

      test('should return false for an invalid regex template match', () {
        expect(NamingUtils.validateName(name: 'AuthRepositoryImplementation', template: '{{name}}(Impl|RepositoryImpl)'), isFalse);
      });
    });
  });
}